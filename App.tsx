...
import { Plus, Trash2, Download, RotateCcw, Image as ImageIcon, CreditCard, User, FileText, Settings, Layout } from 'lucide-react';
...
const INITIAL_STATE: InvoiceData = {
  ...
  items: [
    { id: '1', name: 'Item name', description: 'Description', quantity: 0, price: 0 }
  ],
  discount: 0,
  taxRate: 0,
  footer: {
    paymentInstructions: 'Bank: ABC Bank Account: 1234567890 Routing: 987654321',
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 w-full overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-200 px-[50px] py-4 no-print w-full">
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 leading-tight">InvoicePro</h1>
      ...
      <main className="w-full px-[50px] py-[40px] grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="space-y-8 no-print pb-24">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex items-center gap-4">
                    {data.business.logo && (
                      <img src={data.business.logo} className="w-12 h-12 rounded object-contain border bg-gray-50" />
                    )}
                    <label className="flex-1 cursor-pointer flex items-center justify-center gap-2 px-4 py-2 border-2 border-dashed border-gray-200 rounded-lg hover:border-blue-400 hover:bg-blue-50 transition-all group">
                      ...
                      <span className="text-sm font-medium text-gray-500 group-hover:text-blue-600">Upload Logo</span>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      onChange={(e) => updateBusiness({ brandColor: e.target.value })}
                      className="w-10 h-10 p-0 border-0 rounded-lg cursor-pointer bg-transparent"
                    />
                    <span className="text-xs text-gray-400 font-mono uppercase">{data.business.brandColor}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    updateDetails({ currency: e.target.value, currencySymbol: symbol });
                  }}
                >
          ...
          <section className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 space-y-6">
            ...
            <div className="space-y-4">
              {data.items.map((item) => (
                <div key={item.id} className="p-4 bg-slate-50 rounded-xl space-y-4 group relative">
                  <div className="grid grid-cols-12 gap-4">
                    ...
                    <div className="col-span-3">
                      <div className="relative">
                        <span className="absolute left-3 top-2 text-gray-400 text-sm">{data.details.currencySymbol}</span>
                        <Input 
                          type="number" 
                          className="pl-7"
                          placeholder="Price" 
            ...
            <div className="grid grid-cols-2 gap-8 border-t border-gray-50 pt-6">
              ...
              <div className="bg-slate-50 rounded-xl p-4 space-y-2 flex flex-col justify-center">
                <div className="flex justify-between text-sm text-gray-500">
                  ...
                  <span className="text-black">{data.details.currencySymbol}{data.items.reduce((a, b) => a + (b.quantity * b.price), 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between font-bold text-gray-900 text-lg pt-2 border-t border-gray-200">
                  ...
                  <span className="text-black">
                    ...
                    {(() => {
                      const sub = data.items.reduce((a, b) => a + (b.quantity * b.price), 0);
          ...
          <section className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 space-y-6">
            <div className="flex items-center gap-3 border-b border-gray-50 pb-4">
              ...
              <h2 className="text-lg font-bold text-gray-900">Footer & Signature</h2>
            </div>
            <TextArea 
              label="Payment Instructions" 
              value={data.footer.paymentInstructions} 
              onChange={(e) => updateFooter({ paymentInstructions: e.target.value })} 
            />
            <TextArea 
            ...
            <Input 
              ...
              onChange={(e) => updateFooter({ thankYouMessage: e.target.value })} 
            />
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4 p-1 bg-gray-100 rounded-lg w-fit">
                <button 
                  ...
                  className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${data.footer.signatureType === 'text' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  Type Name
                </button>
                <button 
                  onClick={() => updateFooter({ signatureType: 'image' })}
                  className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${data.footer.signatureType === 'image' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                  Upload Image
                </button>
              </div>
              ...
              {data.footer.signatureType === 'text' ? (
                <Input 
                  ...
                  onChange={(e) => updateFooter({ signatureText: e.target.value })} 
                />
              ) : (
                <label className="cursor-pointer flex items-center justify-center gap-2 px-4 py-6 border-2 border-dashed border-gray-200 rounded-lg hover:border-blue-400 hover:bg-blue-50 transition-all">
                  ...
                  <span className="text-sm font-medium text-gray-500">Upload Signature Image</span>
        ...
        <div className="lg:sticky lg:top-28 w-full">
          <div className="bg-white p-2 rounded-2xl shadow-xl overflow-hidden no-print mb-4 border border-gray-100">
             <div className="px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-50 mb-2">Live Preview</div>
      ...
      <footer className="w-full px-[50px] py-12 text-center text-gray-400 text-sm no-print border-t border-gray-200 mt-12 bg-white">
        &copy; 2025 InvoicePro. Professional invoicing made easy. All rights reserved.
      </footer>
    </div>
  );
};
